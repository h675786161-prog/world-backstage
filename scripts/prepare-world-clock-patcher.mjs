...
const patchStartMarker = `core = replaceOnce(\n    core,\n    "    const identityAnchor = modelText(playerIdentityAnchor, 400);"`;
const patchEndMarker = `core = replaceOnce(\n    core,\n    "        '6. clock_anchor`;
const patchStart = source.indexOf(patchStartMarker);
const patchEnd = source.indexOf(patchEndMarker, patchStart + 1);
if (patchStart < 0 || patchEnd < 0) {
    throw new Error('prepare patcher: prompt patch range not found');
}

const identityNeedle = "    const identityAnchor = modelText(playerIdentityAnchor, 400);";
const identityReplacement = "    const clockAuthorityRule = '绝对日期、星期和精确钟点由插件的确定性时间权威层处理。clock_anchor.mode 必须返回 none；不要猜年月日、星期或当前几点。你只负责 elapsed_minutes（本批 new=true 正文真正经过的时长）和 time_reason。正文已经明确写出的时间证据会由插件代码单独解析。';\n    const identityAnchor = modelText(playerIdentityAnchor, 400);";
const oldClockRules = [
...
const generated = [
    ...
    `    simulation = replaceOnce(simulation, ${JSON.stringify(identityNeedle)}, ${JSON.stringify(identityReplacement)}, 'simulation clock rule constant');`,
    `    simulation = replaceOnce(simulation, ${JSON.stringify(oldClockRules)}, ${JSON.stringify(newClockRules)}, 'simulation clock authority prompt rules');`,
    '    core = core.slice(0, block.start) + simulation + core.slice(block.end);',
    '}',
    '',
].join('\n');

source = source.slice(0, patchStart) + generated + source.slice(patchEnd);
